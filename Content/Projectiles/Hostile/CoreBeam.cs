...
namespace Divergency.Content.Projectiles.Hostile
{
    ...
    public class CoreBeam2 : CoreBeamBase
    {
    ...
    public class LuminaryBeam : CoreBeamBase
    {
        public override void Extra()
        {
            MAX_TIME = 70;
    ...
    public abstract class CoreBeamBase : ModProjectile
    {
        ...
        public override bool PreDraw(ref Color lightColor)
        {
            ...
            DivUtils.Reload(Main.spriteBatch, BlendState.Additive);
            DivUtils.Reload(Main.spriteBatch, SpriteSortMode.Immediate);
            Texture2D bolt = DivUtils.GetExtraTexture("laser");
            Texture2D bolt1 = DivUtils.GetExtraTexture("laser2");
            Texture2D bolt2 = DivUtils.GetExtraTexture("laser3");
            ...
            Texture2D texture = ModContent.Request<Texture2D>("Divergency/Assets/Textures/Line").Value;
            //float scale = Projectile.scale * 2 * mult;
            BeamPacket packet = new BeamPacket();
            packet.Pass = "Texture";
            Vector2 start = Projectile.Center;
            Vector2 end = Projectile.Center + vel * /*DivUtils.TRay.CastLength(Projectile.Center, Projectile.velocity,*/ Main.screenWidth;//);
            ...
            packet2.Add(end + offset * 2 * mult, BeamColor, new Vector2(1 + off, 0));
            packet2.Send();
            DivUtils.Reload(Main.spriteBatch, SpriteSortMode.Deferred);

            texture = ModContent.Request<Texture2D>("Divergency/Assets/Textures/circle_05").Value;
            ...
            Main.spriteBatch.Draw(texture, Projectile.Center - Main.screenPosition, null, Color.LimeGreen, 0, new Vector2(texture.Width, texture.Height) / 2, scale * 0.25f, SpriteEffects.None, 0f);
            ...
            Main.spriteBatch.Draw(texture, end - Main.screenPosition, null, Color.LimeGreen, Projectile.rotation, new Vector2(texture.Width, texture.Height) / 2, scale * 0.15f, SpriteEffects.None, 0f);



            DivUtils.Reload(Main.spriteBatch, BlendState.AlphaBlend);
